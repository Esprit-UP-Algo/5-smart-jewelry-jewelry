...
void MainWindow::exportToPDF() {
    ...
    painter.setPen(Qt::red);  // Couleur du texte en rouge

    // Dessiner le titre centré
    int titleWidth = painter.fontMetrics().width("Liste des employés");
    int titleX = (printer.width() - titleWidth) / 2;
    painter.drawText(titleX, 100, "Liste des employés");

    // Dessiner le cadre autour de la page
    painter.drawRect(50, 50, printer.width() - 100, printer.height() - 100);

    // Définir la police et la taille du texte pour les données du tableau
    QFont tableFont("Arial", 12);
    painter.setFont(tableFont);
    painter.setPen(Qt::black);  // Revenir à la couleur noire

    // Dessiner l'en-tête du tableau
    int headerRow = 140;
    int headerHeight = 20;
    int cellWidth = 100;  // Ajoutez cette ligne pour déclarer cellWidth


    // Dessiner une cellule autour de chaque en-tête
    painter.drawRect(100, headerRow, cellWidth, headerHeight);
    painter.drawText(100, headerRow, cellWidth, headerHeight, Qt::AlignCenter, "Nom");

    painter.drawRect(200, headerRow, cellWidth, headerHeight);
    painter.drawText(200, headerRow, cellWidth, headerHeight, Qt::AlignCenter, "Prénom");

    painter.drawRect(300, headerRow, cellWidth, headerHeight);
    painter.drawText(300, headerRow, cellWidth, headerHeight, Qt::AlignCenter, "CIN");

    painter.drawRect(400, headerRow, cellWidth, headerHeight);
    painter.drawText(400, headerRow, cellWidth, headerHeight, Qt::AlignCenter, "Salaire");
    ...
    if (query.exec()) {
        ...
        int row = 160;  // Réduire la première ligne pour mieux aligner avec le titre
        while (query.next()) {
            ...
            // Dessiner une cellule autour de chaque élément du tableau
            painter.drawRect(100, row, cellWidth, cellHeight);
            painter.drawText(100, row, cellWidth, cellHeight, Qt::AlignCenter, nom);

            painter.drawRect(200, row, cellWidth, cellHeight);
            painter.drawText(200, row, cellWidth, cellHeight, Qt::AlignCenter, prenom);

            painter.drawRect(300, row, cellWidth, cellHeight);
            painter.drawText(300, row, cellWidth, cellHeight, Qt::AlignCenter, QString::number(cin));

            painter.drawRect(400, row, cellWidth, cellHeight);
            painter.drawText(400, row, cellWidth, cellHeight, Qt::AlignCenter, QString::number(salaire));

            // Ajouter d'autres colonnes selon vos besoins
            row += cellHeight; // Augmenter la position Y pour la prochaine ligne
            rowCount++;
...
void MainWindow::marquerJoursFeries(QCalendarWidget *calendrier) {
    ...
    joursFeries.insert(QDate(2023, 1, 2), "Faire un inventaire des bijoux en stock");
    joursFeries.insert(QDate(2023, 1, 7), "Réapprovisionner les articles en rupture de stock");
    joursFeries.insert(QDate(2023, 1, 6), "traiter les commandes reçue");
