...
employe::employe(QString nom,QString prenom,int CIN,int abs,QDate dateE,int salaire,QString mdp,QString RFID)
{
    this->CIN=CIN;
    this->nom=nom;
    this->prenom=prenom;
...
bool employe::ajouter()
{
    ...
    query.prepare("INSERT INTO employe (CIN, nom, prenom, dateE, salaire, abs, mdp,RFID,status) VALUES (:CIN, :nom, :prenom, :dateE, :salaire, :abs, :mdp,:RFID,:status)");
...
bool employe::modifier(int CIN)
{
    ...
    query.prepare("UPDATE employe SET nom = :nom, prenom = :prenom, salaire = :salaire, dateE = :dateE, abs = :abs, mdp= :mdp, RFID = :RFID WHERE CIN = :CIN");
    query.bindValue(":CIN", res);
    query.bindValue(":nom", nom);
    query.bindValue(":prenom", prenom);
    query.bindValue(":dateE", dateE);
...
{
    QSqlQuery query;
    query.prepare("SELECT RFID FROM EMPLOYE WHERE RFID = :uid");
...
{
    QSqlQuery query;
        query.prepare("SELECT NOM FROM EMPLOYE WHERE RFID = :uid");
