...
QSqlQuery query;
    ...
    QRegularExpression nomPattern("^[A-Za-z]+$");

        if (nomPattern.match(nom_c).hasMatch()) {
            query.bindValue(":nom", nom_c);
        }
        else {
            QMessageBox::critical(nullptr, QObject::tr("ajout"),
                            QObject::tr("nom incorrecte !.\n"
                                        "Click Cancel to exit."), QMessageBox::Cancel);

        QRegularExpression prenomPattern("^[A-Za-z]+$");
        }

        if (nomPattern.match(prenom_c).hasMatch()) {
            query.bindValue(":prenom", prenom_c);
        }
        else {
            QMessageBox::critical(nullptr, QObject::tr("ajout"),
                            QObject::tr("prenom incorrecte !.\n"
                                        "Click Cancel to exit."), QMessageBox::Cancel);
        }
